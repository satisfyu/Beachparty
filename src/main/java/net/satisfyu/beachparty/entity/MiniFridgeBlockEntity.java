...
public class MiniFridgeBlockEntity extends BlockEntity implements Inventory, BlockEntityTicker<MiniFridgeBlockEntity>, NamedScreenHandlerFactory {
    ...
    @Override
    public void tick(World world, BlockPos pos, BlockState state, MiniFridgeBlockEntity blockEntity) {
        if (world.isClient) return;
        boolean dirty = false;
        final var recipeType = world.getRecipeManager()
                .getFirstMatch(RecipeRegistry.MINI_FRIDGE_RECIPE_RECIPE_TYPE, blockEntity, world)
                .orElse(null);
